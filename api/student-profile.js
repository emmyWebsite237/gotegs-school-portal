...
export default async function handler(req,res){
  try{
    ...
    if(req.method==='DELETE'){
      ...
      const update=await supabase.from(student.section).update({profile_pic_path:null}).eq('id',student.id);
      if(update.error)return res.status(400).json({error:'Could not remove profile picture: '+update.error.message});
      if(oldPath){try{await supabase.storage.from(BUCKET).remove([oldPath]);}catch(_){} }
      return res.status(200).json({success:true,profile_pic_url:null,profile_pic_removed:true});
    }
    ...
    let buffer;try{buffer=Buffer.from(encoded,'base64');}catch{buffer=null;}
    ...
    const path=`${student.section}/${safeId}-${randomUUID()}.${ext}`;
    ...
    const update=await supabase.from(student.section).update({profile_pic_path:path}).eq('id',student.id);
    ...
    return res.status(200).json({success:true,profile_pic_url:url,profile_pic_removed:false,section:student.section});
  }catch(err){
